...
interface Gutter {
  ...
  createdBy: string;
  phone?: string;    // Add optional fields for agent details
  email?: string;
  name?: string;
  capturedAt: string;
  createdAt: string;
...
const CARD_MIN_HEIGHT = 180; // Reduced from 250

const CardWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <Paper 
    elevation={2} 
    sx={{ 
      p: 2.5, // Reduced padding from 3
      borderRadius: 2, 
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: themeColors.background, py: 4 }}>
      <Container maxWidth="lg">
        ...
        <Card elevation={3} sx={{ borderRadius: 2, overflow: 'hidden' }}>
          ...
          <CardContent sx={{ p: 4 }}>  {/* Reduced padding from 4 */}
            <Grid container spacing={10}>  {/* Reduced spacing from 4 */}
              {/* Location Details */}
              <Grid item xs={12} md={6}>
            ...
            <Divider sx={{ my:10 }} /> {/* Reduced margin from 4 */}
